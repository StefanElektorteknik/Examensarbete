...
// Up to 29 characters
#define BLE_PERIPHERAL_NAME "STEFAN BLE"
...
void setup() {
  ...
  pinMode(CS, OUTPUT);    //Fix the CS pin
  digitalWrite(CS, LOW);
  ...
  config_file();
   ...
   digitalWrite(CS, LOW);
      SPI.transfer(0x43);   //Set the gyro range (GYR_RANGE)
      SPI.transfer(0x04);   //Set the gyr_range and ois_range to +/- 125 dps (degre per second)
   digitalWrite(CS, HIGH); 
  //
...
void send_test_data()
{
  digitalWrite(8, HIGH);
   digitalWrite(CS, LOW);
      SPI.transfer(0x95);
      dummy = SPI.transfer(0xFF);     //Read a dummy byte
      MSBy = SPI.transfer(0xFF);      //Read the data from gyrosensors y-axel (MSB) 
   digitalWrite(CS, HIGH);

    digitalWrite(CS, LOW);
      SPI.transfer(0x94);
      dummy = SPI.transfer(0xFF);   //Read a dummy byte
      LSBy = SPI.transfer(0xFF);    //Read the data from gyrosensors y-axel (LSB) 
   digitalWrite(CS, HIGH);
...
void config_file(){
    digitalWrite(CS, LOW);
      SPI.transfer(0x7E);   //Command Register (CMD)
      SPI.transfer(0xB6);   //Writing the softreset. It means that all the configurations will be declared as default.
    digitalWrite(CS, HIGH);
    ...
    digitalWrite(CS, LOW);
      SPI.transfer(0x7C);   //Power mode configuration register (PWR_CONF)       
      SPI.transfer(0x02);   //Disable advanced power save and fast power up.
    digitalWrite(CS, HIGH); //Enabled FIFO read in low power mode
    
    delayMicroseconds(500);

    digitalWrite(CS, LOW);
      SPI.transfer(0x59);   //Start initialization address (INIT_CTRL)
      SPI.transfer(0x00);   //Starting the initialization
    digitalWrite(CS, HIGH);

       delay(50);
       
    digitalWrite(CS, LOW);
      SPI.transfer(0x5B);   //Address that is sorting the initialization data prat by part(INIT_ADDR_0)
      SPI.transfer(0x00);   //Vary the base address
      SPI.transfer(0x00);   //Reserve address
    digitalWrite(CS, HIGH);
    
    digitalWrite(CS, LOW);
      SPI.transfer(0x5E);   //Initialzation register (INIT_DATA)
      for(int i=0;i<8192;i++){
        SPI.transfer(init_file[i]);   // Register to initilize the data
      }
    digitalWrite(CS, HIGH);
   delay(50);  
   ...
   digitalWrite(CS, LOW);
      SPI.transfer(0x59);   //Start initialization address (INIT_CTRL)
      SPI.transfer(0x01);   //Stop the initialization 
   digitalWrite(CS, HIGH);

   digitalWrite(CS, LOW);
      SPI.transfer(0x7C);   //Power mode configuration register (PWR_CONF)
      SPI.transfer(0x03);   //Disable fast power up.
   digitalWrite(CS, HIGH);  //Enabled FIFO read in low power mode and advanced power save    
   
   digitalWrite(CS, LOW);
      SPI.transfer(0x7C);   //Power mode configuration register (PWR_CONF) 
      SPI.transfer(0x02);   //Disable fast power up and advanced power save.
   digitalWrite(CS, HIGH);  //Enabled FIFO read in low power mode.     

   digitalWrite(CS, LOW);
      SPI.transfer(0x2F);   //Page number for feature configuration and outfput register (FEAT_PAGE)
      SPI.transfer(0x00);   //Use the default data
   digitalWrite(CS, HIGH);
   ...
   digitalWrite(CS, LOW);
      SPI.transfer(0x58);   //Data interupt mapping(INT_MAP_DATA)
      SPI.transfer(0x04);   //Data ready to interupt for INT1
   digitalWrite(CS, HIGH);
   ...
   digitalWrite(CS, LOW);
      SPI.transfer(0x40);   // Accelerometer configuration (ACC_CONF)
      SPI.transfer(0x88);   // Using odr_100, osr4_avg1 and preformance optimized.
      SPI.transfer(0x01);
   digitalWrite(CS, HIGH);
   ...
   digitalWrite(CS, LOW);
      SPI.transfer(0x42);   //Gyroscope configurations (GYR_CONF)
      SPI.transfer(0x88);   //Using odr_100, osr4, power optimized in noise and preformance optimized in filter.    
      SPI.transfer(0x08);
   digitalWrite(CS, HIGH);
   ...
   digitalWrite(CS, LOW);
      SPI.transfer(0x7D);   //Power mode controll register(PWR_CTRL)
      SPI.transfer(0x06);   //Accelerometer and Gyroscope enabled   
   digitalWrite(CS, HIGH);
